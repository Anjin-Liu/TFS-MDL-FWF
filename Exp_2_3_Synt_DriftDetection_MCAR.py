...
def mcar_drift_detection(run_method_list, drift_type='gau_mean', alpha=0.05, num_test_PerDriftDelta=150, top_k=3):
    
    m = 500
    mv_config = {}
    if drift_type=='gau_mean' or drift_type=='gau_cov':
        n = 4
        for i in range(4):
            mv_config[i] = 0.2
    elif drift_type=='uni_mean':
        n = 10
        for i in range(5):
            mv_config[i] = 0.2
    ...
    for delta in drift_delta:
        ...
        for i in range(num_test_PerDriftDelta):
            #r_seed = delta_idx * num_test_PerDriftDelta + i + 100000
            r_seed = delta_idx * num_test_PerDriftDelta + i + 1
...
#            plt.show()
            ...
            if 'mfkchi2_fuzzy_gau' in run_method_list:
                print('mfkchi2_fuzzy_gau')
                np.random.seed(0)
                mfkchi2_fuzzy_result, mfkchi2_miss_fuzzy_gau, mfkchi2_full_fuzzy_gau = perform_mfkmean_chi2_test(
                        train_miss, test_miss, train_full, test_full, alpha, mfkchi2_miss_fuzzy_gau, mfkchi2_full_fuzzy_gau, apply_fuzzy='Gaussion', top_k=top_k)
            ...
            if 'mfkchi2_fuzzy_tri' in run_method_list:
                print('mfkchi2_fuzzy_tri')
                np.random.seed(0)
                mfkchi2_fuzzy_result, mfkchi2_miss_fuzzy_tri, mfkchi2_full_fuzzy_tri = perform_mfkmean_chi2_test(
                        train_miss, test_miss, train_full, test_full, alpha, mfkchi2_miss_fuzzy_tri, mfkchi2_full_fuzzy_tri, apply_fuzzy='Triangle', top_k=top_k)
                detection_result[delta_idx, 1+run_method_counter:3+run_method_counter] = detection_result[delta_idx, 1+run_method_counter:3+run_method_counter] + mfkchi2_fuzzy_result
                run_method_counter = run_method_counter + 2
...
#        detection_result = gau_mcar_drift_detection(run_method_list, alpha, num_test_PerDriftDelta, drift_type='cov')
        detection_result = mcar_drift_detection(run_method_list, drift_type, alpha, num_test_PerDriftDelta, top_k=top_k)
