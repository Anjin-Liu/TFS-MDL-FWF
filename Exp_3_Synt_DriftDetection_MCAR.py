...
def uni_mcar_drift_detection(run_method_list, alpha=0.05, num_test_PerDriftDelta=150):
    ...
    detection_result = np.zeros([drift_delta_size, len(run_method_list)*2+1])
...
def gau_mcar_drift_detection(run_method_list, alpha=0.05, num_test_PerDriftDelta=150, drift_type='mean'):
...
#                plt.scatter(train_full[:,0], train_full[:,1])
#                plt.scatter(test_full[:,0], test_full[:,1])
#                plt.show()
...
if __name__ == "__main__":
    ...
    '''
    print('Exp3')
    num_test_PerDriftDelta = 500
    alpha = 0.05
    run_method_list = ['mww', 'kchi2', 'mfkchi2_fuzzy_gau', 'mfkchi2_fuzzy_tri', 'mfkchi2_crisp', 'ME', 'MMD', 'QuantTree']
    #run_method_list = ['QuantTree']
    #dataset_list = ['uni', 'gau_mean', 'gau_cov']
    dataset_list = ['gau_mean']
    top_k=3
    ...
    if 'uni' in dataset_list:
        uni_detection_result = uni_mcar_drift_detection(run_method_list, alpha, num_test_PerDriftDelta)
        uni_detection_result = pd.DataFrame(uni_detection_result, columns=uni_columns)
        uni_detection_result.to_csv('Results/uni_detection.csv', index=False)
    
    if 'gau_mean' in dataset_list: 
        gau_detectionMean_result = gau_mcar_drift_detection(run_method_list, alpha, num_test_PerDriftDelta, drift_type='mean')
        gau_detectionMean_result = pd.DataFrame(gau_detectionMean_result, columns=uni_columns)
        gau_detectionMean_result.to_csv('Results/gau_detectionMean_result.csv', index=False)
        
    if 'gau_cov' in dataset_list:
        gau_detectionCov_result = gau_mcar_drift_detection(run_method_list, alpha, num_test_PerDriftDelta, drift_type='cov')
        gau_detectionCov_result = pd.DataFrame(gau_detectionCov_result, columns=uni_columns)
        gau_detectionCov_result.to_csv('Results/gau_detectionCov_result.csv', index=False)
        
    if 'poi_mean' in dataset_list:
        pass
    
    if 'poi_rho' in dataset_list:
        pass
    ...
    '''
    if 'poi_mean' in dataset_list:
        gau_detectionCov_result = gau_mcar_drift_detection(run_method_list, drift_type='cov')
        gau_detectionCov_result = pd.DataFrame(gau_detectionCov_result, columns=uni_columns)
        gau_detectionCov_result.to_csv('Results/gau_detectionCov_result.csv', index=False)
